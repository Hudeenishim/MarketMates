...
// Patch handleUpdateProduct to properly update local state in demo mode
code = code.replace(
  /const handleUpdateProduct = async \(e: React\.FormEvent\) => \{([\s\S]*?)if \(demoMode\) \{([\s\S]*?)\} else \{([\s\S]*?)try \{([\s\S]*?)\}/,
...
console.log("Patched buttons in demo mode");
