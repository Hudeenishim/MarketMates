...
// 2. Add the inner component MapControls above RiderDashboard
...
code = code.replace(
  ...
  (match) => match.replace("<Map", "<MapControls onLocationSelect={(lat, lng) => handleMapClick({ detail: { latLng: { lat, lng } } })} />\n              <Map")
);
